...
type Grabber struct {
	...
	karmadaControllerManager []string
}
...
func NewMetricsGrabber(ctx context.Context, c clientset.Interface) (*Grabber, error) {
	...
	for _, pod := range podList.Items {
		if regKarmadaControllerManager.MatchString(pod.Name) {
			grabber.karmadaControllerManager = append(grabber.karmadaControllerManager, pod.Name)
		}
	}
	return &grabber, nil
}

// GrabMetricsFromKarmadaControllerManager returns metrics from controller manager
func (g *Grabber) GrabMetricsFromKarmadaControllerManager(ctx context.Context) (*testutil.Metrics, error) {
	...
	for _, podName := range g.karmadaControllerManager {

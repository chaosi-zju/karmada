...
func runUploadAdminKubeconfig(r workflow.RunData) error {
	...
	for _, secret := range secretList {
		...
		if err != nil {
			return fmt.Errorf("failed to create/update kubeconfig secret '%s', err: %w", secret.Name, err)
		}
	}
...
func generateComponentKubeconfigSecrets(data InitData, configString string) []*corev1.Secret {
	...
	for karmadaComponentName, component := range componentList {
		if component != nil {
			secrets = append(secrets, generateKubeconfigSecret(util.ComponentKubeconfigSecretName(karmadaComponentName), data.GetNamespace(), configString))
		}
	}
